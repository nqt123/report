var DFT = function ($) {
    ...
    var bindSubmit = function () {
        $('form#new-campaigns-mail').validationEngine('attach', {
            ...
            onValidationComplete: function (form, status) {
                if (status) {
                    $.get('/mail-campaigns/validate?' + $.param({fieldId: 'name', fieldValue: form.find('input#name').val(), setting: form.find('#edit_setting').selectpicker('val')}), function (resp) {
                        if (!resp[1]) {
                            form.find('input#name').validationEngine('showPrompt', 'Tên chiến dịch và cấu hình này đã được sử dụng', 'error', 'topRight', true);
                        } else {
                            _AjaxData('/mail-campaigns', 'POST', form.getData(), function (resp) {
    ...
    var bindClick = function () {
        $(document).on('click', '#new-campaigns-mail .btn.btn-primary', function () {
    ...
    return {
        init: function () {
            $('#new-campaigns-mail #edit_sendDate').datetimepicker({minDate: moment().add(10, 'minutes')})
            $.validationEngineLanguage.allRules['DuplicateCheck'] = {
                "url": "/mail-campaigns/validate",
                "extraDataDynamic": '#edit_setting',
        ...
        uncut: function () {
            ...
            $(document).off('click', '#new-campaigns-mail .btn.btn-primary');
