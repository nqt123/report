var DFT = function ($) {
    ...
    var getFilter = function () {
        ...
        _Ajax("/report-chat-by-month?"+ $.param(filter),'GET',{}, function(resp){
            if(resp.code==200){
                if (resp.data.length) {
                    initTable(resp.data)
                }else
        ...
        _.each(datas, function(data, i){
            ...
            var tags= _.Tags([
                {tag:'tr', attr: {id: data._id}, childs: [
                    {tag:'td', attr:{class: 'text-center'}, content: data._id.month + "/" + data._id.year},//thang
    ...
    var bindClick = function () {
        ...
        $('#exportexcel').on('click', function (event) {
            ...
            $(this).attr('download', todaysDate + '_BaoCaoChatTheoThang.xls')
