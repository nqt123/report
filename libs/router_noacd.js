...
module.exports = function routers(app) {
    ...
    app.post('/login', function (req, res) {
        ...
        _Users.findOne({$and: [{$or: [{name: _body.name}, {email: _body.name}]}, {password: _body.password}]}, function (error, user) {
            if (_.isNull(user)) return res.status(200).send({code: 404, message: 'Sai tên đăng nhập hoặc mật khẩu'});
    ...
    app.get('/customer-excel', function (req, res) {
        ...
        var _cl = [];
        _CustomerFields.find({status: 1}).sort({weight: 1, displayName: 1}).exec(function (error, fields) {
