...
module.exports = function routers(app) {
        ...
        _Users.findOne({ $and: [{ $or: [{ name: _body.name }, { email: _body.name }] }, { password: _body.password }] }, function (error, user) {
            ...
            acdPublish.sendLoginRequest(user, _body.deviceId, function (message) {
                if (message && message.resultCode == 0) {
                    ...
                    req.xhr ? res.status(200).send({ code: 200 }) : res.redirect('/');
                    ...
                    _Mail.aggregate([
                        ...
                        if (agents.length) {
                            ...
                            _async.each(agents, function (a, cb) {
                                _mailBulk.find({ _id: _.convertObjectId(a._id) }).update({ $set: { agent: _.convertObjectId(a.agent) } });
                                cb();
                            }, function (error) {
    ...
    app.get('/customer-excel', function (req, res) {
        var options = {
            filename: path.join(_rootPath, 'assets', 'export', 'customer-schema-' + _moment().format('DD-MM-YYYY') + '.xls'),
        ...
        _async.waterfall([
            function(next){
                _Company.distinct('companyProfile', {}, next);
            },
            function(ids, next){
                _CompanyProfile.find({_id: {$in: ids}}, function(err, profiles){
                    var fieldsId = [];
            ...
            function(ids,next){
                _CustomerFields.find({_id: {$in: ids}, status: 1 }).sort({ weight: 1, displayName: 1 }).exec(next);
            }
            ...
            _async.eachSeries(fields, function (feld, cb) {
                ...
                if (feld.isRequired) _style.font['color'] = { argb: "FF0000" };
                _cl.push({
